    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try {
            ...
            for (Part part : request.getParts()) {
                if ("productSubphoto[]".equals(part.getName())) {
                    String fileName = extractFileName(part);
                    VariantInfo v = new VariantInfo(productID, fileName, productColor, size, material);
                    ProductDAOimpl d = new ProductDAOimpl(DBContext.getConn());
                    boolean f = d.addVariant(v);

                    HttpSession session = request.getSession();
                    if (f) {
                        String path = getServletContext().getRealPath("") + "img" + File.separator +"subphoto";
